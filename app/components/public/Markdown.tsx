...
import { MarkdownLink } from '~/components/public/MarkdownLink'
import type { HTMLProps } from 'react'
import { getSingletonHighlighter as shikiGetHighlighter } from 'shiki/bundle-web.mjs'
...
const markdownComponents: Record<string, React.FC> = {
  ...
  code: function Code({ className, ...rest }: HTMLProps<HTMLElement>) {
    return <code className={`${className ?? ` ${className}`}`} {...rest} />
  },
  iframe: (props) => (
...
function CodeBlock(props: React.HTMLProps<HTMLPreElement>) {
  ...
  if (
    ...
    lang = ((props.children.props as any).className as string).replace(
      'language-',
      '',
    )
  }
  ...
  return (
    <div className={`w-full max-w-full relative`} style={props.style}>
